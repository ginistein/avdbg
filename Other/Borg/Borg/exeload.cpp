...
void fileloader::patchfile(dword file_offs,dword num,byte *dat)
{
	...
	{
		MessageBox(g_hMainWnd,"File I/O Error - Invalid Handle for writing","Borg Message",MB_OK);
		return;
	}
...
void fileloader::patchoep(void)
{
	if(exetype!=PE_EXE)
		return;
	peh->entrypoint_rva=g_options.oep.offs-peh->image_base;
...
void fileloader::reloadfile(dword file_offs,dword num,byte *dat)
{
	...
	if(efile==INVALID_HANDLE_VALUE)
	{
		MessageBox(g_hMainWnd,"File I/O Error - Invalid Handle for reading","Borg Message",MB_OK);
		return;
	}
	SetFilePointer(efile,file_offs,NULL,FILE_BEGIN);
...
void fileloader::readcomfile(dword fsize)
{
	...
	g_options.dseg=g_options.loadaddr.segm;
	dta.addseg(g_options.loadaddr,fsize,fbuff,code16,NULL);
	dta.possibleentrycode(g_options.loadaddr);
	g_options.mode16=true;
	g_options.mode32=false;
	dio.setcuraddr(g_options.loadaddr);
	g_scheduler.addtask(dis_code,priority_definitecode,g_options.loadaddr,NULL);
	g_scheduler.addtask(nameloc,priority_nameloc,g_options.loadaddr,"start");
...
void fileloader::readsysfile(dword fsize)
{
	...
	while(!done)
	{
		t.assign(g_options.loadaddr.segm,devhdr);
		g_scheduler.addtask(dis_dataword,priority_data,t,NULL);
		g_scheduler.addtask(dis_dataword,priority_data,t+2,NULL);
		g_scheduler.addtask(dis_dataword,priority_data,t+4,NULL);
		g_scheduler.addtask(dis_dataword,priority_data,t+6,NULL);
		g_scheduler.addtask(dis_dataword,priority_data,t+8,NULL);
		g_scheduler.addtask(dis_dataword,priority_data,t+18,NULL);
		if((((word *)(fbuff+devhdr))[3])&&((((word *)(fbuff+devhdr))[3])<fsize))
		{
			t.assign(g_options.loadaddr.segm,((word *)(fbuff+devhdr))[3]+devhdr);
			if(t.offs)
			{
				g_scheduler.addtask(dis_code,priority_definitecode,t,NULL);
				g_scheduler.addtask(nameloc,priority_nameloc,t,"strategy");
			}
		}
		if((((word *)(fbuff+devhdr))[4])&&((((word *)(fbuff+devhdr))[4])<fsize))
		{
			t.assign(g_options.loadaddr.segm,((word *)(fbuff+devhdr))[4]);
			if(t.offs)
			{
				g_scheduler.addtask(dis_code,priority_definitecode,t,NULL);
				g_scheduler.addtask(nameloc,priority_nameloc,t,"interrupt");
			}
		}
		g_scheduler.addtask(windowupdate,priority_window,nlptr,NULL);
...
void fileloader::readpefile(dword peoffs)
{
	...
	start_addr=nlptr;
	g_options.dseg=g_options.loadaddr.segm;
	sseg.segm=g_options.loadaddr.segm;
	sseg.offs=0;
	pestart=&fbuff[peoffs];
	peh=(peheader *)pestart;
	g_options.loadaddr.offs=peh->image_base; // bugfix build 14
	...
	pdata=(peobjdata *)(pestart+24+peh->nt_hdr_size);	//IMAGE_OPTIONAL_HEADER32后面紧跟着的就是区段了
	for(i=0;i<peh->objects;i++)		//以区块的数量为循环次数
	{
		peobjdone=false;
		if((pdata[i].rva==peh->exporttable_rva)  // export info 如果rva=导出表的rva
			||((peh->exporttable_rva>pdata[i].rva)&&(peh->exporttable_rva<pdata[i].rva+pdata[i].phys_size)))	//或者在这个区段的RVA中，则判断这个区段是导出表
		{
			expdir=(peexportdirentry *)&fbuff[pdata[i].phys_offset+peh->exporttable_rva-pdata[i].rva];	//获取导出表（一个IMAGE_EXPORT_DIRECTORY）
			t.assign(g_options.loadaddr.segm,peh->image_base+peh->exporttable_rva);		//	设置段基地址和逻辑地址
			g_scheduler.addtask(dis_datadword,priority_data,t,NULL);
			g_scheduler.addtask(dis_datadword,priority_data,t+4,NULL);
			g_scheduler.addtask(dis_dataword,priority_data,t+8,NULL);
			g_scheduler.addtask(dis_dataword,priority_data,t+10,NULL);
			g_scheduler.addtask(dis_datadword,priority_data,t+12,NULL);
			g_scheduler.addtask(dis_datadword,priority_data,t+16,NULL);
			g_scheduler.addtask(dis_datadword,priority_data,t+20,NULL);
			g_scheduler.addtask(dis_datadword,priority_data,t+24,NULL);
			g_scheduler.addtask(dis_datadword,priority_data,t+28,NULL);
			...
			g_scheduler.addtask(dis_datastring,priority_data,t,NULL);
			...
			lef.assign(g_options.loadaddr.segm,expdir->funcaddrrva+peh->image_base);
			leo.assign(g_options.loadaddr.segm,expdir->ordsaddrrva+peh->image_base);
			len.assign(g_options.loadaddr.segm,expdir->nameaddrrva+peh->image_base);
			while(numsymbols)
			{ g_scheduler.addtask(dis_datadword,priority_data,lef,NULL);
			g_scheduler.addtask(dis_dataword,priority_data,leo,NULL);
			g_scheduler.addtask(dis_datadword,priority_data,len,NULL);
			chktable[onaddr[0]]=1;
			t.assign(g_options.loadaddr.segm,peh->image_base+fnaddr[onaddr[0]]);
			g_scheduler.addtask(dis_export,priority_export,t,(char *)&fbuff[(*nnaddr)+pdata[k].phys_offset-pdata[k].rva]);
			t.assign(g_options.loadaddr.segm,(*nnaddr)+peh->image_base);
			g_scheduler.addtask(dis_datastring,priority_data,t,NULL);
			...
			t.assign(g_options.loadaddr.segm,peh->image_base+fnaddr[onaddr[0]]);
			g_scheduler.addtask(dis_exportcode,priority_definitecode,t,NULL);
			numsymbols--;
			onaddr++;
			nnaddr++;
			lef+=4;
			...
			if(expdir->numfunctions>expdir->numnames)
			{
				for(j=0;j<expdir->numfunctions;j++)
				{
					if(!chktable[j])
					{
						...
						if(fnaddr[j])
						{
							t.assign(g_options.loadaddr.segm,fnaddr[j]+peh->image_base);
							g_scheduler.addtask(dis_ordexport,priority_export,t,newimpname);
							g_scheduler.addtask(dis_code,priority_definitecode,t,NULL);
						}
					}
				}
		...
		if((pdata[i].rva==peh->importtable_rva) // import info	导入表的处理
			||((peh->importtable_rva>pdata[i].rva)&&(peh->importtable_rva<pdata[i].rva+pdata[i].phys_size)))
		{
			...
			{
				t.assign(g_options.loadaddr.segm,
					peh->image_base+peh->importtable_rva+j*sizeof(struct peimportdirentry));
				g_scheduler.addtask(dis_datadword,priority_data,t,NULL);
				g_scheduler.addtask(dis_datadword,priority_data,t+4,NULL);
				g_scheduler.addtask(dis_datadword,priority_data,t+8,NULL);
				g_scheduler.addtask(dis_datadword,priority_data,t+12,NULL);
				g_scheduler.addtask(dis_datadword,priority_data,t+16,NULL);
				for(k1=0;k1<peh->objects;k1++)
				{
				...
				t.assign(g_options.loadaddr.segm,impdir[j].namerva+peh->image_base);
				g_scheduler.addtask(dis_datastring,priority_data,t,NULL);
				...
				while(*imphint)
				{
					if((*imphint)&0x80000000)
					{
						...
						strcat(impbuff,inum);
						strcpy(newimpname,impbuff);
						t.assign(g_options.loadaddr.segm,impaddr);
						g_scheduler.addtask(dis_ordimport,priority_import,t,newimpname);
					}
					else
					{
						t.assign(g_options.loadaddr.segm,impaddr);
						g_scheduler.addtask(dis_import,priority_import,t,(char *)&fbuff[(*imphint)+2+pdata[k1].phys_offset-pdata[k1].rva]);
					}
					t.assign(g_options.loadaddr.segm,peh->image_base+(*imphint));
					g_scheduler.addtask(dis_dataword,priority_data,t,NULL);
					g_scheduler.addtask(dis_datastring,priority_data,t+2,NULL);
					t.assign(g_options.loadaddr.segm,impaddr);
					g_scheduler.addtask(dis_datadword,priority_data,t,NULL);
					t.assign(g_options.loadaddr.segm,impaddr2);
					g_scheduler.addtask(dis_datadword,priority_data,t,NULL);
					imphint++;
					impaddr+=4;
					impaddr2+=4;
		...
		{
			// RESOURCE_DATA;
			if((pdata[i].phys_size)&&(g_options.loadresources))
			{
		...
		if(pdata[i].rva==peh->fixuptable_rva) // fixup info
		{
			...
			while(per->rva)
			{
				...
				{
					t.assign(g_options.loadaddr.segm,((dword)(rdata[0])&0x0fff)+per->rva+peh->image_base);
		...
		{
			// INIT_DATA;
			if((pdata[i].phys_size)&&(g_options.loaddata))
			{
				sseg.offs=pdata[i].rva+peh->image_base;
				dta.addseg(sseg,pdata[i].phys_size,&fbuff[pdata[i].phys_offset],data32,NULL);
			}
			if((pdata[i].virt_size>pdata[i].phys_size)&&(g_options.loaddata))
			{
		...
		if(!start_addr.segm)
		{
			start_addr.assign(g_options.loadaddr.segm,pdata[i].rva+peh->image_base);
			dio.setcuraddr(start_addr);
		}
	}
	start_addr.assign(g_options.loadaddr.segm,peh->entrypoint_rva+peh->image_base);
	g_options.oep=start_addr;
	dio.setcuraddr(start_addr);
	g_scheduler.addtask(dis_code,priority_definitecode,start_addr,NULL);
	g_scheduler.addtask(nameloc,priority_nameloc,start_addr,"start");	//输出start:标签
	g_scheduler.addtask(windowupdate,priority_window,nlptr,NULL);
}
...
void fileloader::readmzfile(dword fsize)
{
	...
	if(!nrelocs)
	{
		MessageBox(g_hMainWnd,"Relocation table is empty\nThis file is probably packed"
	...
	while(nrelocs)
	{
		...
		if((ritem[0])||(ritem[1]))
		{
			...
			rchange[0]+=(word)g_options.loadaddr.segm;
			rtable[mzh->numrelocs-nrelocs]=rchange[0];
		}
		else
			rtable[mzh->numrelocs-nrelocs]=g_options.loadaddr.segm;
		nrelocs--;
	}
	qsort(rtable,mzh->numrelocs,2,mzcmp);
	sseg=g_options.loadaddr;
	g_options.dseg=g_options.loadaddr.segm;   // need to look for better value for dseg later
	ip=g_options.loadaddr;
	ipaddr=(((word)mzh->csip)+((word)(mzh->csip/0x10000L)+g_options.loadaddr.segm)*16L+g_options.loadaddr.offs)&0xfffff;
	for(nrelocs=0;nrelocs<mzh->numrelocs;nrelocs++)
	{
		if(rtable[nrelocs]!=sseg.segm)
		{
			...
			if((saddr<taddr)&&(sseg.segm>=g_options.loadaddr.segm))
			{
				dta.addseg(sseg,taddr-saddr,fbuff+mzh->headersize*16
					+(sseg.segm-g_options.loadaddr.segm)*16L,code16,NULL);
				dta.possibleentrycode(sseg);
				...
				for(nr=0;nr<mzh->numrelocs;nr++)
				{
					...
					{
						rchange=(word *)(&poffs[ritem[0]+ritem[1]*16L]);
						if(((byte *)rchange>=fbuff+mzh->headersize*16+(sseg.segm-g_options.loadaddr.segm)*16L)
							&&((byte *)rchange<fbuff+mzh->headersize*16+(sseg.segm-g_options.loadaddr.segm)*16L+taddr-saddr))
							reloc.addreloc(sseg+(dword)((byte *)rchange-
							(fbuff+mzh->headersize*16+(sseg.segm-g_options.loadaddr.segm)*16L)),RELOC_SEG);
					}
				}
				sseg.segm=tseg.segm;
			}
	...
	if((sseg.segm-g_options.loadaddr.segm)*16UL<fs)
	{
		saddr=sseg.segm*16L+sseg.offs;
		if(ipaddr>=saddr)
		{
		...
		dta.addseg(sseg,fs-(sseg.segm-g_options.loadaddr.segm)*16L,
			fbuff+mzh->headersize*16+(sseg.segm-g_options.loadaddr.segm)*16L,code16,NULL);
		for(nr=0;nr<mzh->numrelocs;nr++)
		{
			ritem=(word *)(&roffs[(mzh->numrelocs-nr)*4L]);
			if((ritem[0])||(ritem[1]))
	...
	g_options.mode32=false;
	dio.setcuraddr(ip);
	g_scheduler.addtask(dis_code,priority_definitecode,ip,NULL);
	g_scheduler.addtask(nameloc,priority_nameloc,ip,"start");
	g_scheduler.addtask(windowupdate,priority_window,nlptr,NULL);
	if(mzh->numrelocs)
		delete rtable;
}
...
void fileloader::readnefile(dword neoffs)
{
	...
	char iname[80];
	g_options.dseg=g_options.loadaddr.segm;
	sseg.assign(g_options.loadaddr.segm,0);
	nestart=&fbuff[neoffs];
	neh=(neheader *)nestart;
	if(!neh->csip)
	{
		MessageBox(g_hMainWnd,"No entry point to executable - assume that Executable"
	...
	// set up disassembly
	g_options.loadaddr.assign(stable[(neh->csip>>16)-1],neh->csip&0xffff);
	dio.setcuraddr(g_options.loadaddr);
	g_scheduler.addtask(dis_code,priority_definitecode,g_options.loadaddr,NULL);
	g_scheduler.addtask(nameloc,priority_nameloc,g_options.loadaddr,"start");
	g_scheduler.addtask(windowupdate,priority_window,nlptr,NULL);
...
void fileloader::readbinfile(dword fsize)
{
	...
	g_options.dseg=g_options.loadaddr.segm;
	dta.addseg(g_options.loadaddr,fsize,fbuff,g_options.mode32 ? code32:code16,NULL);
	dta.possibleentrycode(g_options.loadaddr);
	dio.setcuraddr(g_options.loadaddr);
	g_scheduler.addtask(dis_code,priority_definitecode,g_options.loadaddr,NULL);
	g_scheduler.addtask(nameloc,priority_nameloc,g_options.loadaddr,"start");
...
void fileloader::leafnodesummary(byte *data,char *resname,dword image_base,dword rtype)
{
	...
	t.assign(g_options.loadaddr.segm,leaf->datarva+image_base);
	// bugfix to third arg - build 14
	dta.addseg(t,leaf->size,&rawdata[leaf->datarva-pdatarva],resourcedata,resname);
	switch(rtype)
	{
	case 5: // dialog
		g_scheduler.addtask(dis_dialog,priority_data,t,resname);
		break;
	case 6: // stringtable
		g_scheduler.addtask(dis_stringtable,priority_data,t,resname);
		break;
	default:
		break;
	}

...
bool range::checkblock(void)
{
	if(top==nlptr)
	{
		MessageBox(g_hMainWnd,"Set top of block first","Borg Disassembler",MB_OK);
		return false;
	}
	if(bottom==nlptr)
	{
		MessageBox(g_hMainWnd,"Set bottom of block first","Borg Disassembler",MB_OK);
		return false;
	}
	if(top>bottom)
	{
		MessageBox(g_hMainWnd,"Block empty ?","Borg Disassembler",MB_OK);
		return false;
	}
	return true;
}
...
void range::undefine(void)
{
	if(!checkblock())
		return;
	g_dsm.undefineblock(top,bottom);
}
...
void range::settop(void)
{
	...
	MessageBox(g_hMainWnd,"Top marked","Borg Disassembler",MB_OK);
}
...
void range::setbottom(void)
{
	...
	MessageBox(g_hMainWnd,"Bottom marked","Borg Disassembler",MB_OK);
}

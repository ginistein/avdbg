...
void disio::argoversingle(void)
{
	lptr outhere;
	findcurrentaddr(&outhere);
	g_dsm.disargoversingle(outhere);
}
...
void disio::argoveroffsetdseg(void)
{
	lptr outhere;
	findcurrentaddr(&outhere);
	g_dsm.disargoveroffsetdseg(outhere);
}
...
void disio::makelongdouble(void)
{
	...
	if(outhere==curraddr)
		subitem=dsmnull;
	g_dsm.disdatalongdouble(outhere);
}
...
void disio::dosstring(void)
{
	...
	if(outhere==curraddr)
		subitem=dsmnull;
	g_dsm.disdatadosstring(outhere);
}
...
void disio::generalstring(void)
{
	...
	if(outhere==curraddr)
		subitem=dsmnull;
	g_dsm.disdatageneralstring(outhere);
}
...
void disio::setcuraddr(lptr loc)
{
	...
	if(tblock!=NULL)
		...
		subitem=dsmnull;
	tblock=g_dsm.nextiter_code(tblock);
	if(tblock!=NULL)
	{
	...
	SetScrollPos(g_hMainWnd,SB_VERT,(int)sbarpos,true);
}
	...
	if(tblock!=NULL)
	{
		if(arg1)
		{
			...
			case ARG_IMM32:
				...
				data=tblock->data+tblock->length;
				data-=4;
				outhere.assign(g_options.dseg,((dword *)(&data[0]))[0]);
				if(dta.findseg(outhere)!=NULL)
				{
					retstack.push(curraddr);
					setcuraddr(outhere);
...
void disio::scroller(dword amount)
{
	...
	if((amount==1)&&(usersel<=nScreenRows-3))
	{
		...
		InvalidateRect(g_hMainWnd,&r,true);
		return;
	}
	// simple move on screen
...
void disio::dumpblocktofile(char *fname,bool printaddrs)
{
	...
	{
		MessageBox(g_hMainWnd,"File Creation Failed","Borg Disassembler Alert",MB_OK);
		return;
	}

...
void disasm::initnewdsm(dsmitem *newdsm,lptr loc,dsmitemtype typ)
{
	...
	newdsm->modrm=0;
	newdsm->mode32=g_options.mode32;
	newdsm->flags=0;
...
{
	int i=0;
	while((procnames[i].num!=0)&&(procnames[i].num!=g_options.processor))
		i++;
	m_itable=procnames[i].tab;
}
...
void disasm::disargoveroffsetdseg(lptr loc)
{
	...
	if(findit!=NULL)
	{
		if((findit->addr==loc)&&(findit->type==dsmcode))
		{
			...
			dio.updatewindowifinrange(loc);
			if((g_options.mode32)&&(findit->length>=4))
			{
				j.assign(g_options.dseg,((dword *)(&findit->data[findit->length-4]))[0]);
				xrefs.addxref(j,loc);
			}
		}
	}
...
void disasm::codeseek(lptr loc)
{
	...
	do{
		...
		if(dcount>1000)
		{
			...
			g_scheduler.addtask(seek_code,priority_continuation,loc,NULL);
			return;
		}
		}
	} while (doneblock);
	// decode it.
	g_scheduler.addtask(seek_code,priority_aggressivesearch,loc+1,NULL);
	g_scheduler.addtask(dis_code,priority_possiblecode,loc,NULL);
}
...
void disasm::disblock(lptr loc)
{
	...
	while(!doneblock)
	{
		...
		if(disasmcount>50)
		{
			g_scheduler.addtask(dis_code,priority_continuation,loc,NULL);
			return;
		}
		memset(ibuff,0,20);
		// check if already done.
		...
		tblock=decodeinst(ibuff,mcode,loc,TABLE_MAIN,g_options.mode32,0);
		if(tblock!=NULL)
		{
...
dsmitem *disasm::decodeinst(byte *ibuff,byte *mcode,lptr loc,byte tabtype,bool omode32,int depth)
{
	...
	if(tabtype==TABLE_EXT2)
		cbyte=ibuff[2];
	if((tabtype==TABLE_EXT2)&&(g_options.processor==PROC_Z80))
		cbyte=ibuff[3];
	while(m_itable[tablenum].table!=NULL)  // search tables
	{
		righttable=true;
		if((m_itable[tablenum].type!=tabtype)||(m_itable[tablenum].minlim>cbyte)||
			(m_itable[tablenum].maxlim<cbyte)) righttable=false;
		if(((tabtype==TABLE_EXT)||(tabtype==TABLE_EXT2))&&(ibuff[0]!=m_itable[tablenum].extnum))
			righttable=false;
		if((tabtype==TABLE_EXT2)&&(ibuff[1]!=m_itable[tablenum].extnum2))
			righttable=false;
		if(righttable)
		{
			insttab=m_itable[tablenum].table;      // need to search this now
			instnum=0;
			mbyte=cbyte;
			fupbyte=false;
			if(m_itable[tablenum].divisor)
				mbyte=(byte)(mbyte/m_itable[tablenum].divisor);
			if(m_itable[tablenum].mask)
				mbyte=mbyte&m_itable[tablenum].mask;
...
#pragma warn -sig
#endif
			length=1+arglength(a1,mcode[1+m_itable[tablenum].modrmpos],mcode[2+m_itable[tablenum].modrmpos],flgs,true)
				+arglength(a2,mcode[1+m_itable[tablenum].modrmpos],mcode[2+m_itable[tablenum].modrmpos],flgs,true)
				+arglength(a3,mcode[1+m_itable[tablenum].modrmpos],mcode[2+m_itable[tablenum].modrmpos],flgs,true)
				+m_itable[tablenum].modrmpos;
#ifdef __BORLANDC__
#pragma warn +sig
#endif
			...
			while((insttab[instnum].name!=NULL)||(insttab[instnum].instbyte)||(insttab[instnum].processor))
			{
				...
				else if((insttab[instnum].instbyte==mbyte)&&(insttab[instnum].processor&g_options.processor))
				{
					// found it
					...
					else
					{
						...
						newdsm->modrm=(byte)(1+m_itable[tablenum].modrmpos);
						newdsm->data=mcode;
...
#pragma warn -sig
#endif
						length=1+arglength(a1,mcode[1+m_itable[tablenum].modrmpos],mcode[2+m_itable[tablenum].modrmpos],flgs,newdsm->mode32)
							+arglength(a2,mcode[1+m_itable[tablenum].modrmpos],mcode[2+m_itable[tablenum].modrmpos],flgs,newdsm->mode32)
							+arglength(a3,mcode[1+m_itable[tablenum].modrmpos],mcode[2+m_itable[tablenum].modrmpos],flgs,newdsm->mode32)
							+m_itable[tablenum].modrmpos;
#ifdef __BORLANDC__
#pragma warn +sig
#endif
						// addition for table extensions where inst is part of modrm byte
						if(((tabtype==TABLE_EXT)||(tabtype==TABLE_EXT2))&&((length==1)||(g_options.processor==PROC_Z80)))
							length++;
						if(g_options.processor==PROC_Z80)
						{
							if(tabtype==TABLE_EXT2)
								length++;
						...
						if(flgs&(FLAGS_JMP|FLAGS_CALL|FLAGS_CJMP))
						{
							...
							case ARG_RELIMM:
								...
								g_scheduler.addtask(dis_code,priority_definitecode,j,NULL);
								...
								if(g_options.mode32)
								{
									if((newdsm->data[0]==0xff)&&(newdsm->data[1]==0x25)&&(tabtype==TABLE_EXT))
									{
										...
										if(import.isname(j))
										{
											...
											g_scheduler.addtask(namecurloc,priority_nameloc,loc,impname);
										}
									}
								}
								g_scheduler.addtask(dis_jumptable,priority_definitecode,loc,NULL);
								break;
							default:
								break;
							}
						...
						case ARG_IMM32:
							if(reloc.isreloc(loc+length-4))
							{
								...
								dta=mcode+length-4;
								j.assign(g_options.dseg,((dword *)(&dta[0]))[0]);
								xrefs.addxref(j,newdsm->addr);
							}
							break;
						case ARG_IMM:
...
void disasm::disjumptable(lptr loc)
{
	...
	if((!name.isname(t))&&(!expt.isname(t))&&(!import.isname(t)))
		g_scheduler.addtask(namecurloc,priority_nameloc,t,tablename);
	xrefs.addxref(t,loc);
	if(itable)
	{
		itables++;
		...
		strcat(tablename,tablenum);
		if(!name.isname(it))
			g_scheduler.addtask(namecurloc,priority_nameloc,it,tablename);
		index.segm=loc.segm;
	...
	for(i=0;i<numjumps;i++)
	{
		...
		if(g_options.mode32)
		{
			...
			g_scheduler.addtask(dis_datadsoffword,priority_data,t+i*indexamount,NULL);
			g_scheduler.addtask(dis_code,priority_definitecode,xr,NULL);
			xrefs.addxref(xr,t+i*indexamount);
		}
		else
		{
			xr.assign(loc.segm,((word *)(dblock->data+(t-dblock->addr)+i*indexamount))[0]);
			g_scheduler.addtask(dis_dataword,priority_data,t+i*indexamount,NULL);
			g_scheduler.addtask(dis_code,priority_definitecode,xr,NULL);
			xrefs.addxref(xr,t+i*indexamount);
		}
	}
...
void disasm::disxref(lptr loc)
{
	...
	if(!((expt.isname(loc))||(import.isname(loc))||(name.isname(loc))))
	{
		...
		g_scheduler.addtask(namecurloc,priority_nameloc,loc,locname);
	}
	dio.updatewindowifinrange(loc);
}

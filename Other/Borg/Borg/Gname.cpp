...
void gname::addname(lptr loc,char *nm)
{
	...
	if(checkdup!=NULL)
		if(checkdup->addr==loc)
		{
			...
			delete newname;
			g_dsm.delcomment(loc,dsmnameloc);
			if(strlen(checkdup->name))
				g_dsm.discomment(loc,dsmnameloc,(byte *)checkdup->name);
			else
			{
				delfrom(checkdup);
			}
		...
		addto(newname);
		g_dsm.discomment(loc,dsmnameloc,(byte *)newname->name);

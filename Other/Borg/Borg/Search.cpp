...
void searchengine(void)
{
	...
	DialogBox(g_hInst,MAKEINTRESOURCE(Search_Dialog),g_hMainWnd,(DLGPROC)searchbox);
...
void dosearch(HWND hdwnd,search_type searchtype,bool fromstart,lptr s_seg)
{
	...
	sbox=CreateDialog(g_hInst,MAKEINTRESOURCE(S_Box),hdwnd,(DLGPROC)searchingbox);
	switch(searchtype)
	{
	case SEARCH_STR:
	...
	if(found)
		g_scheduler.addtask(user_jumptoaddr,priority_userrequest,s_seg,NULL);
	DestroyWindow(sbox);
}
/************************************************************************
* searchmore                                                            *
...
void searchmore(void)
{
	...
	s_seg+=g_dsm.getlength(s_seg);
	dosearch(g_hMainWnd,lastsearchtype,false,s_seg);
	g_scheduler.continuethread();
}

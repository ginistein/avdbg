	...
	while(chker!=NULL)
	{
		if(insegloc(addit,chker->addr))
		{
			...
			if(!addit->size)
			{
				...
				MessageBox(g_hMainWnd,warning,"Borg Warning",MB_ICONEXCLAMATION|MB_OK);
				return;
			}
			else
			{
...
#endif
	g_scheduler.addtask(dis_segheader,priority_segheader,loc,NULL);
...
void dataseg::possibleentrycode(lptr loc)
{
	...
	if(g_options.codedetect&CD_AGGRESSIVE)
		g_scheduler.addtask(seek_code,priority_aggressivesearch,loc,NULL);
	while(length)
	{
		if((g_options.codedetect&CD_PUSHBP)&&(length>3))
		{
		...
		if((g_options.codedetect&CD_EAXFROMESP)&&(length>4))
		{
			if(findd->data[loc-findd->addr]==0x55)  // push bp
			{
		...
		if((g_options.codedetect&CD_MOVBX)&&(length>2)) // mov bx,sp
		{
...
void dataseg::segheader(lptr loc)
{
	...
	tmpc=new byte[80];
	strcpy((char *)tmpc,"-----------------------------------------------------------------------");
	g_dsm.discomment(loc,dsmsegheader,tmpc);
	tmpc=new byte[80];
	t=loc.segm;
	...
	strcat((char *)tmpc,(char *)tmp2);
	g_dsm.discomment(loc,(dsmitemtype)(dsmsegheader+1),tmpc);
	tmpc=new byte[80];
	switch(findd->typ)
	{
	case code16:
	...
	g_dsm.discomment(loc,(dsmitemtype)(dsmsegheader+2),tmpc);
	tmpc=new byte[80];
	strcpy((char *)tmpc,"-----------------------------------------------------------------------");
	g_dsm.discomment(loc,(dsmitemtype)(dsmsegheader+3),tmpc);
}

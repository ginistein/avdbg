...
void decrypt::process_dec(dword dec_id)
{
	...
	if(!loading_db)
		g_dsm.undefineblock(patch->dec_start,patch->dec_end);
...
void decrypt::exepatch(dword dec_id)
{
	...
	while(cpos<=patch->dec_end)
	{
		...
		if(cpos>patch->dec_end)
			break;
		doitval=g_floader.fileoffset(cpos);
		// write patch
		switch(plen)
		{
		case 1:
			g_floader.patchfile(doitval,1,pseg->data+(cpos-pseg->addr));
			break;
		case 2:
			g_floader.patchfile(doitval,2,pseg->data+(cpos-pseg->addr));
			break;
		case 4:
			g_floader.patchfile(doitval,4,pseg->data+(cpos-pseg->addr));
			break;
		default:
			g_floader.patchfile(doitval,1,pseg->data+(cpos-pseg->addr));
			break;
...
void decrypt::process_reload(dword dec_id)
{
	...
	while(cpos<=patch->dec_end)
	{
		...
		{
		case 1:
			g_floader.reloadfile(doitval,1,pseg->data+(cpos-pseg->addr));
			break;
		case 2:
			g_floader.reloadfile(doitval,2,pseg->data+(cpos-pseg->addr));
			break;
		case 4:
			g_floader.reloadfile(doitval,4,pseg->data+(cpos-pseg->addr));
			break;
		default:
			g_floader.reloadfile(doitval,1,pseg->data+(cpos-pseg->addr));
			break;

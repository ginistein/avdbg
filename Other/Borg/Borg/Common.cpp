...
void demangle(char **nme)
{
	...
	while(name[i])
	{
		...
		else if(!_strnicmp((char *)&name[i],"$bctr",5))
		{
			k=0;
			while((buff[k]!=':')&&(k<20))
			{ buff[j++]=buff[k++];
		...
		else if(!_strnicmp((char *)&name[i],"$bdtr",5))
		{
			k=0;
			buff[j++]='~';
			while((buff[k]!=':')&&(k<20))
		...
		else if(!_strnicmp((char *)&name[i],"$bdla",5))
		{
			strcpy((char *)&buff[j],"delete");
			j+=6;
			i+=4;
		}
		else if(!_strnicmp((char *)&name[i],"$bnwa",5))
		{
			strcpy((char *)&buff[j],"new");
			j+=3;
			i+=4;
		}
		else if(!_strnicmp((char *)&name[i],"$bdele",6))
		{
			strcpy((char *)&buff[j],"delete");
			j+=6;
			i+=5;
		}
		else if(!_strnicmp((char *)&name[i],"$bnew",5))
		{
			strcpy((char *)&buff[j],"new");
			j+=5;
			i+=4;
...
void init_ofn(OPENFILENAME *ofn)
{
	...
	szFilesave[0]=0;
	cbString=LoadString(g_hInst,IDS_FILTERSTRING,szFilter,sizeof(szFilter));

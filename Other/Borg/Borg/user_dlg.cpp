...
void exportsviewer(void)
{
	...
	{
		MessageBox(g_hMainWnd,"There are no exports in the list","Borg Message",MB_OK);
	}
	else
	{
		DialogBox(g_hInst,MAKEINTRESOURCE(Exports_Viewer),g_hMainWnd,(DLGPROC)exportsbox);
	}
	g_scheduler.continuethread();
}
	...
	case WM_COMMAND:
		{
			...
			case IDC_JUMPTOBUTTON:
				g_scheduler.addtask(user_jumptoaddr,priority_userrequest,t->addr,NULL);
				EndDialog(hdwnd,NULL);
				return true;
			default:
				break;
...
void importsviewer(void)
{
	...
	{
		MessageBox(g_hMainWnd,"There are no imports in the list","Borg Message",MB_OK);
	}
	else
	{
		DialogBox(g_hInst,MAKEINTRESOURCE(Imports_Viewer),g_hMainWnd,(DLGPROC)importsbox);
	}
	g_scheduler.continuethread();
}
...
void namelocation(void)
{
	...
	g_scheduler.stopthread();
	nme=NULL;
	DialogBox(g_hInst,MAKEINTRESOURCE(Get_Name),g_hMainWnd,(DLGPROC)getnamebox);
	if(nme!=NULL)
	{
		dio.findcurrentaddr(&loc);
		g_scheduler.addtask(namecurloc,priority_userrequest,loc,nme);
	}
	g_scheduler.continuethread();
}
...
void namesviewer(void)
{
	...
	{
		MessageBox(g_hMainWnd,"There are no names in the list","Borg Message",MB_OK);
	}
	else
	{
		DialogBox(g_hInst,MAKEINTRESOURCE(Names_Viewer),g_hMainWnd,(DLGPROC)namesbox);
	}
	g_scheduler.continuethread();
}
	...
	case WM_COMMAND:
		{
			...
			case NAMES_DELETE:
				name.delname(t->addr);
				g_scheduler.addtask(user_repeatnameview,priority_userrequest,nlptr,NULL);
				EndDialog(hdwnd,NULL);
				return true;
			case NAMES_RENAME:
				nme=NULL;
				DialogBox(g_hInst,MAKEINTRESOURCE(Get_Name),hdwnd,(DLGPROC)getnamebox);
				if(nme!=NULL)
					g_scheduler.addtask(namecurloc,priority_userrequest,t->addr,nme);
				g_scheduler.addtask(user_repeatnameview,priority_userrequest,nlptr,NULL);
				EndDialog(hdwnd,NULL);
				return true;
			default:
...
{
	g_scheduler.stopthread();
	DialogBox(g_hInst,MAKEINTRESOURCE(Block_Dialog),g_hMainWnd,(DLGPROC)blockbox);
	g_scheduler.continuethread();
}
...
void xrefsviewer(void)
{
	...
	if(findit==NULL)
	{
		MessageBox(g_hMainWnd,"Unable to find any xrefs for the location","Borg Message",MB_OK);
	}
	else if(findit->addr!=xtmp.addr)
	{
		MessageBox(g_hMainWnd,"There are no xrefs for the current location in the list","Borg Message",MB_OK);
	}
	else
	{
		DialogBox((struct HINSTANCE__ *)g_hInst,MAKEINTRESOURCE(Xrefs_Viewer),g_hMainWnd,(DLGPROC)xrefsbox);
	}
	g_scheduler.continuethread();
}
	...
	case WM_COMMAND:
		{
			...
			case IDC_OKBUTTON:
				...
				return true;
			case IDC_JUMPTOBUTTON:
				g_scheduler.addtask(user_jumptoaddr,priority_userrequest,currentsel->refby,NULL);
				EndDialog(hdwnd,NULL);
				return true;
			case XREFS_DELETE:
				g_scheduler.addtask(user_delxref,priority_userrequest,currentsel->refby,NULL);
				g_scheduler.addtask(windowupdate,priority_window,nlptr,NULL);
				if(numberofitems>1)
					g_scheduler.addtask(user_repeatxrefview,priority_userrequest,nlptr,NULL);
				EndDialog(hdwnd,NULL);
				return true;
			default:
				break;
...
{
	g_scheduler.stopthread();
	DialogBox(g_hInst,MAKEINTRESOURCE(Seg_Viewer),g_hMainWnd,(DLGPROC)segbox);
	g_scheduler.continuethread();
}
...
void changeoep(void)
{
	if(g_floader.exetype!=PE_EXE)
	{ MessageBox(g_hMainWnd,"Can only change the oep for PE files","Borg",MB_OK);
	return;
	}
	g_scheduler.stopthread();
	DialogBox(g_hInst,MAKEINTRESOURCE(OEP_Editor),g_hMainWnd,(DLGPROC)getoepbox);
	g_scheduler.continuethread();
}
...
BOOL CALLBACK getoepbox(HWND hdwnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			g_scheduler.addtask(nameloc,priority_userrequest,g_options.oep,"");
			g_options.oep=loc;
			g_scheduler.addtask(nameloc,priority_userrequest,g_options.oep,"start");
			if(SendDlgItemMessage(hdwnd,IDC_PATCHOEP,BM_GETCHECK,(WPARAM)0,(LPARAM)0))
				patchit=true;
			else 
				patchit=false;
			if(g_options.readonly&&patchit)
			{
...
{
	g_scheduler.stopthread();
	DialogBox(g_hInst,MAKEINTRESOURCE(Jaddr_Editor),g_hMainWnd,(DLGPROC)getjaddrbox);
	g_scheduler.continuethread();
}
...
{
	g_scheduler.stopthread();
	DialogBox(g_hInst,MAKEINTRESOURCE(Comment_Editor),g_hMainWnd,(DLGPROC)getcommentbox);
	g_scheduler.continuethread();
}
...
BOOL CALLBACK getcommentbox(HWND hdwnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_COMMAND:
		{
			...
			case IDOK:
				...
				g_scheduler.addtask(user_delcomment,priority_userrequest,loc,NULL);
				if(strlen(newcomment))
					g_scheduler.addtask(user_addcomment,priority_userrequest,loc,newcomment);
				return true;
			case IDCANCEL:
				EndDialog(hdwnd,NULL);
				return true;
	...
	case WM_INITDIALOG:
		...
		titem.addr=loc;
		titem.type=dsmnull;
		g_dsm.findnext(&titem);
		tblock=g_dsm.nextiterator();
		if(tblock!=NULL)

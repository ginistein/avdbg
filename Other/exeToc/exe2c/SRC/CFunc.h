...
struct AsmCode
{
    ...
    static AsmCode* new_AsmCode()	//SlickEdit 不支持列出所有的new方法，就用这个方法了
    {
        AsmCode* p = new AsmCode(); 
        ZeroMemory(p, sizeof(AsmCode));
		p->esp_level = ESP_UNKNOWN;			// 初始化为未知
		p->esp_level_next = ESP_UNKNOWN;	// 初始化为未知
        p->iAddRemove = 0;
        return p;
    }
private:
...
public:
    ...
    bool m_IfLibFunc;		// 是否是有定义的库函数
    CFuncType* m_functype;	// 函数类型信息
	...
	ea_t	m_head_off;	// 函数开始位置地址
    ea_t	m_end_off;	// 函数终止位置地址
...
public:
	...
	void	hxd_prtout_Asm(CXmlOutPro* out);		//	把CFunc按汇编显示
	void	Hxd_prtout_Pcode(CXmlOutPro* out);		//  把CFunc按伪代码信息
...
public:
	...
	bool Step9_Var_Split();			// 寄存器变量拆分，如EAX 分成EAX_1,EAX_2

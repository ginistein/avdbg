...
void FileLoader::get_exetype()	// 获取EXE类型
{
...
bool FileLoader::load(PSTR fname)
{
	DWORD pe_offset;
	DWORD num;
	DWORD fsize;
	...
	efile=CreateFile(fname,GENERIC_READ,1,&securityatt,OPEN_EXISTING,0,NULL);	// 判断文件是否存在
	if ( efile==INVALID_HANDLE_VALUE )
		return FALSE;

	if ( GetFileType(efile)!=FILE_TYPE_DISK )return FALSE;	// 判断文件类型

	get_exetype();	// 获取EXE类型

    if (exetype != PE_EXE)	// 现在仅支持 PE 格式
    	return FALSE;

	fsize=GetFileSize(efile,NULL);	// 获取文件大小
	fbuff=new BYTE[fsize];			// 创建缓冲区

	SetFilePointer(efile,0x00,NULL,FILE_BEGIN);	// 设置读取指针到文件开始处
	ReadFile(efile,fbuff,fsize,&num,NULL);		// 文件读入缓冲区
...
bool  IfInWorkSpace(ea_t off)	// 检查EXE2C程序工作的区间
{

...
#include "CXmlPrt.h"
void CXmlList::prtprtout(PSTR str, CXmlOutPro* prt)
{
...
void CXmlList::Display(PSTR pstr, I_COLOROUT* iColorOut, XMLTYPE curw_xmltype, PVOID curw_p)
{
	if ( curw_xmltype != XT_invalid && curw_p != NULL
	 && (curw_xmltype == this->m_xmltype && curw_p == this->m_p)
	    )
	{
		iColorOut->SetBKColor(RGB(255,255,0));
	}

	COLORREF k_color = XmlType_2_Color(this->m_xmltype);	// 获取各种类型的颜色
	UINT k_pos = this->m_posfrom;

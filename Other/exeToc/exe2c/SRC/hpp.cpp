...
void One_Line_pre(PSTR lbuf); // 处理所有以 # 开始的行
...
BOOL hpp_init()
{
	...
	strcpy(g_incpath, GetMyExePath());// 得到程序路径
	strcat(g_incpath, "\\inc\\");	  // 设置inc获取路径
...
CHpp::CHpp()
{
	m_FuncTypeList = new FuncTypeList;	// 函数类型列表
}
...
//  处理#开始的行，如#include "stdio.h" 与 #define One,1
//	已经去掉了最后的换行符
//	已经把\连接的行接起来了
void CCInfo::OneLine( PSTR lbuf, PSTR &pnext )
{
	...
	{	//	检查是不是多行的 func define
		...
		PSTR pf = p;
		VarTypeID id = get_DataType(pf);	// 得到函数的返回类型
		if (id)	//	第一个量肯定是数据类型
		{
			char buf1[80];
			skip_eos(pf);
			...
			if (cc == enum_unknown)
				cc = enum_stdcall;			// 参数调用方式默认为 _stdcall
			else
			{
				skip_eos(pf);
				get_1part(buf1,pf);
			...
			if (*pf == '(')		//	这下，肯定是func define 了
			{
				...
				pnewfunc->m_callc = cc;				// 参数调用方式
				pnewfunc->m_retdatatype_id = id;	// 函数返回类型
				pnewfunc->m_pname = new_str(buf1);	// 函数名称
				//this->do_func_proto
...
bool LoadIncFile(PSTR fname)
{
	...
	PSTR p = pInfo->m_buf;
	PSTR plast = p + pInfo->m_len;	// 文件结束的位置
	while (p < plast)
	{
		
		PSTR pnext = NULL;
		pInfo->OneLine(p, pnext);	//	一般情况下，OneLine不会动pnext,如果它是多行，就会把pnext指向最后
...
{	
	...
	if (memcmp(p,"define",6) == 0)	// 宏定义
	{
		...
		get_1part(buf,p);
		skip_space(p);
		do_define(buf,p);	// 添加到g_DefineList列表中
		return;
	}

...
typedef struct
{
	DWORD	tbl1_rva;	// 函数名称索引地址表,用于保存函数名称
	DWORD	dummy1;		// 04
	DWORD	dummy2;		// 08
	DWORD	dllname_rva;	//0c
	DWORD	tbl2_rva;	// 地址表，用于保存函数地址
	//size of = 0x14
}IMP_0;
...
//	DWORD	impsize = peh->import_datasize;
    ...
    for (IMP_0*	pimp0 = (IMP_0*)pimp; pimp0->tbl1_rva != 0; pimp0++)
	{
		...
		log_prtl("Loading DLL %s ",pDLLname);

		PDWORD	p1 = PDWORD(pestart+pimp0->tbl1_rva);	// 函数名称索引表
		PDWORD	p2 = PDWORD(pestart+pimp0->tbl2_rva);	// 函数地址索引表
		DWORD d;

		while ((d = *p1) != 0)	// 函数名称地址不为零
		{
...
ea_t	Find_Main(ea_t start)	// 查询main地址
{
	ea_t p = start;
	if (Peek_W(p) == 0x8B55
		&& Peek_W(p+3) == 0xFF6A
...
BOOL	Valid_ea(ea_t ea)	// 判断是否为有效地址
{

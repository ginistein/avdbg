...
void CExprManage_cpp_exit()
{
	...
	SAFE_DELETE(g_VarTypeManage);
}

CVarTypeMng::CVarTypeMng()
{
	list = new VarTypeList;	// 变量列表
	nextfreeid = 1;	
	...
	NewBaseVarType(4, "float");	//	id = 7

	// 非基类
	NewSignedVarType(id_BYTE, "char");	
...
CVarTypeMng::~CVarTypeMng()
{
	...
	while (pos)
	{
		...
		case vtt_signed:
			...
			break;
		case vtt_funcpoint:
			//if (p->m_funcpoint.pFuncType != (CFuncType*)1)
				SAFE_DELETE(p->m_funcpoint.pFuncType);
			break;
        case vtt_simple:
            break;
		}
...
VarTypeID	CVarTypeMng::NewBaseVarType(SIZEOF opsize, PSTR name)
{
	...
	pnew->id = id;
	pnew->type = vtt_base;				// 基类
	pnew->m_base.name = new_str(name);	// 基类名称
	pnew->m_base.opsize = opsize;		// 长度
...
VarTypeID	CVarTypeMng::NewTypeDef(VarTypeID id0, PSTR name)
{	
	assert(id0);
...
VarTypeID CVarTypeMng::VarType_Name2ID(PSTR name)
{
	//	先借用一下下面程序吧
...
// 获取基数信息
VarTypeID get_DataType_bare(PSTR &p)
{
	PSTR savp = p;
	char buf[80];
	buf[0] = '\0';
...
VarTypeID CVarTypeMng::FirstDataType(PSTR &partern)
{
    assert(list);
...
VarTypeID CVarTypeMng::NewUnknownStruc(PSTR strucname)
{
	POSITION pos = list->GetHeadPosition();
	while (pos)
	{

...
{
	OP_TYPE mode;		//操作数类型 OP_Register, ...
	BYTE    rwflag;		//读写标识   0:Unknown	1:Read 2:Write 3:Access
	BYTE    opersize;	//操作数大小 1:BYTE, 2:WORD, 4:DWORD, 8:double DWORD
	union
	{
		struct
		{
			BYTE    seg_index;	// 段寄存器编号
...
{
	enum OPCODETYPE        opcode;	// 指令类型 C_MOV...

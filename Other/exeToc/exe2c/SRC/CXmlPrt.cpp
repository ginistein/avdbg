...
void CXmlOutPro::prtslen(PSTR s, int len)
{
	char buf[280];
    assert(len<280);
    strncpy(buf,s,len);
...
CXmlPrt::CXmlPrt()
{
	...
	m_curword_type = XT_invalid;	// 初始化数据类型
	m_curword_p = NULL;
}
...
{
    PSTR pstr = this->m_str->GetString();
...
{//输入二唯的行列，返回一唯的pos
    PSTR pstr_org = this->m_str->GetString();
    PSTR pstr = pstr_org;
    if (pstr == NULL)
        return 0;
...
int CXmlPrt::GetLineCount()	// 获取当前行数
{
	PSTR pstr = this->m_str->GetString();
    if (pstr == NULL)
        return 0;
...
UINT GetPosFromXY(PSTR p0, int x, int y)
{
	PSTR p = p0;
	while (y)
	{

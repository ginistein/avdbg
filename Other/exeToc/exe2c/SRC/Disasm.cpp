...
INSTRUCTION	instruction0FH[] =
{
	...
	{ C_RDTSC,		"RDTSC",	D_NONE,	D_NONE,	D_NONE },	//0x31,
...
void st_IDA_OUT::output(PSTR buf)
{
    ...
    {
        CString tem;
		tem.Format("%s%s%s",this->Par1Ptr,this->Par1SegPrefix,this->Par1Str);
        if (!tem.IsEmpty())
            retn += tem;
    }
    //par2
    {
        CString tem;
		tem.Format("%s%s%s",this->Par2Ptr,this->Par2SegPrefix,this->Par2Str);
        if (!tem.IsEmpty())
        {
            retn += ',';
            retn += tem;

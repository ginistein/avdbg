...
PSTR RegName(DWORD off, BYTE opsize)
{
	static char s[20];
	switch (opsize)
	{
...
// 就是说，不替换
PSTR CExprManage::BareVarName(VAR* v)
{
    if (v->type == v_Tem)
    {
        PSTR retn = v->thevar->GetName();
        IsBadReadPtr(retn,1);
        return retn;
    }
    PSTR retn = VarName(v);
...
M_t* CExprManage::GetVarByName_1(MLIST* list, PSTR varname)
{
...
M_t* CExprManage::GetVarByName(PSTR varname)
{
    return this->GetVarByName_1(this->vList, varname);
}

PSTR GetBeautyImmedValue(DWORD n)	// 显示立即数，大于9以十六进制表示
{
    static char s[80];
    if (n < 10)
        sprintf(s,"%d",n);
...
void Cstr_fmt(PSTR dst, PSTR src)
{
	int n = 0;
	dst[n++] = '\"';
...
void 	CExprManage::prt_var_Immed(VAR* v, CXmlOutPro* out)
{
    ...
    if ( id_p != 0 && g_VarTypeManage->If_Based_on_idid(id_p,id_char))
    {
        ...
        if ( ! if_valid_ea((ea_t)v->d))	//	奇怪
        {
            ...
            out->prtt(GG_VarType_ID2Name(id1).GetBuffer());
        ...
        PSTR pstr = (PSTR)ea2ptr((ea_t)v->d);
...
PSTR CExprManage::VarName_Immed(VAR* v)
{
    if (v->thevar == NULL || g_VarTypeManage->is_simple(v->thevar->m_DataTypeID))
...
PSTR CExprManage::VarName(VAR* v)
{
    assert(v->type != v_Tem);
...
void CExprManage::prt_var_declares(CXmlOutPro* out)
{
	...
	while (pos)
	{
        ...
        out->prtt(GG_VarType_ID2Name(p->m_DataTypeID).GetBuffer());
        out->XMLend(XT_DataType);
...
M_t* CExprManage::AddRef_with_name(en_MTTYPE type, DWORD off, DWORD size, PSTR tj_name)
...
//	告诉一声CExprManage，这个var 有人用，确保所有var都会产生一个thevar
void CExprManage::AddVarRef(VAR* pvar)
{	
	switch (pvar->type)
	{
	case v_Invalid: // 不可用
    ...
    case v_Reg:		// 寄存器		
        {
            PSTR pname = RegName(pvar->reg, pvar->opsize);

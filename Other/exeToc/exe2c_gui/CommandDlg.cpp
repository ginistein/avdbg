...
BOOL CCommandDlg::PreTranslateMessage(MSG* pMsg) 
{
	...
	if(pMsg->message==WM_KEYDOWN && pMsg->wParam==13)	// 回车处理
	{
		CString m_strCommand;
		GetDlgItem(IDC_COMMAND)->GetWindowText(m_strCommand);
        ::g_EXE2C->DoCommandLine(m_strCommand.GetBuffer());
...
void CCommandDlg::AddLine(PSTR str)
{
	...
	if (p != NULL)
	{
		p->ReplaceSel(buf);	// 替换当前选择内容	
	}
	return;
}

void CCommandDlg::ClearLog()	// 清空Log
{

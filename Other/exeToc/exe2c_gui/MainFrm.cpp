...
CMainFrame::CMainFrame()
{
    ...
    g_EXE2C = NEW_EXE2C();	// 宏处理，调用dll，生成Cexe2c对象
	g_EXE2C->Init(this);
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndMyBar1.Create(_T("控制条"), this, CSize(350,100),TRUE,123))
	{
...
void CMainFrame::ShowFun()	// 显示函数名称
{
    ...
    while (h)
    {
		...
		if (info.name[0] != 0
		&& info.m_IfLibFunc==false	// 库函数不在列表中显示
		)
		{
...
void CMainFrame::OnMenuClosePro()
{
	...
	g_Mfm->ClearFun();			// 清空函数列表框
	g_EXE2C->ClearFuncList();	// 清空 m_func_list

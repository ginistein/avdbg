...
BEGIN_MESSAGE_MAP(CRorTextView, CView)
	...
	ON_COMMAND(ID_EDIT_COPY, OnEditCopy)			// 复制
    ON_COMMAND(ID_EDIT_SELECT_ALL, OnEditSelectAll)	// 全选
...
void CRorTextView::ResetView()	// 初始化参数
{
	m_nTopLine = 0;
	m_nOffsetChar = 0;
	m_nLineHeight = -1;
...
#define SHOW_ONE 1
    ...
    CDC cacheDC;
    VERIFY(cacheDC.CreateCompatibleDC(pdc));	// 创建一个与指定设备兼容的内存设备上下文环境

    if (m_pCacheBitmap == NULL)
    {
        m_pCacheBitmap = new CBitmap;
        VERIFY(m_pCacheBitmap->CreateCompatibleBitmap(pdc, rcClient.Width(), nLineHeight));	// 创建与指定的设备环境相关的设备兼容的位图
    }
...
// 输出文本
void __stdcall CRorTextView::ColorOut(PSTR str, long len, COLORREF textcolor)
{

...
private:
	...
	void DrawLineHelperImpl(CDC *pdc, CPoint &ptOrigin, LPCTSTR pszChars, int nCount);
	void DrawLineHelperImpl_1(CDC *pdc, CPoint &ptOrigin, LPCTSTR pszChars, int nCount);
...
protected:
	...
	void DrawLineHelper(CDC *pdc, CPoint &ptOrigin, LPCTSTR pszChars, int nCount, CPoint ptTextPos);
	virtual void DrawSingleLine(CDC *pdc, int nLineIndex);
    void func1(CDC *pdc, TEXTBLOCK *pBuf, int I, int nLength, CPoint& origin,LPCTSTR pszChars, int nLineIndex);
...
public:
    ...
    virtual void __stdcall ColorOut(PSTR str, long len, COLORREF textcolor);
